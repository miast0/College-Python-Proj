...
def view_one_employee(employee_id_list, f_name_list, l_name_list, email_list, salary_list):
    ...
    while view_one_employee_loop == True:
        ...
        if id_selection in employee_id_list:
            ...
            employee_id = employee_id_list.index(id_selection)
            print("\nID\t\tF_name\tL_name\t\tEmail\t\tSalary\n")
            print(employee_id_list[employee_id], " ", f_name_list[employee_id], " ", l_name_list[employee_id], " ",
                  email_list[employee_id], " ", salary_list[employee_id] , "\n")
...
def edit_salary(employee_id_list, f_name_list, l_name_list, salary_list):
    ...
    while edit_salary_loop == True:
        ...
        if user_selection in employee_id_list:
            employee_id = employee_id_list.index(user_selection)
            print("\nID\t\tF_name\tL_name\t\tSalary\n")

            print(employee_id_list[employee_id], " ", f_name_list[employee_id], " ", l_name_list[employee_id], " ",
                  salary_list[employee_id] , "\n")
            new_salary = input("Change salary from " + str(salary_list[employee_id]) + " to: ")
            #this will set the employees salary to the new salary value
            salary_list[employee_id] = float(new_salary)
            user_selection = int(input("\nSelect an input\n"
                                       "1. Run again\n"
                                       "2. Return to menu\n>>> "))
            if user_selection == 1:
...
def add_employee(employee_id_list, f_name_list, l_name_list, email_list, salary_list):
    print("Add Employee\n")
    add_employee_loop = True
    while add_employee_loop == True:
        add_employee_loop = False
        # these two lines add the new employees first and last name to the list.
        ...
        #This function creates a new ID for the user
        id_generator(employee_id_list)
        #This function creates an email for the user
        ...
        while salary_loop == True:
            ...
            try:
                ...
                salary_loop = True
        print("\nID\t\tF_name\tL_name\t\tEmail\t\tSalary\n")
        #This will print hte new employeses data, to show what was created
        print(employee_id_list[len(employee_id_list)-1], " ", f_name_list[len(employee_id_list)-1], " ", l_name_list[len(employee_id_list)-1], " ",email_list[len(employee_id_list)-1], " ",
              salary_list[len(employee_id_list)-1] , "\n")
        time.sleep(1)
        user_selection = int(input("\nSelect an input\n"
                                   "1. Run again\n"
                                   "2. Return to menu\n>>> "))
...
def id_generator(employee_id_list):
    ...
    while id_gen_loop == True:
        id_gen_loop = False
        #This creates a temporary ID of size 5 with a random interger from 0-9
        temp_id = str(randint(0, 9)) + str(randint(0, 9)) + str(randint(0, 9)) + str(randint(0, 9)) +str(randint(0, 9))
        #Check if the created ID is already in use
...
def email_generator(f_name_list, l_name_list, email_list):
    #creates a temporary begining of email, using first name and last name seprated by dot
    ...
    while email_check_loop == True:
        ...
        i += 1
        #if the temporary email is in use, it adds 1 to the end of the email, and checks again.
...
def delete_employee(employee_id_list, f_name_list, l_name_list, email_list, salary_list):
    ...
    while delete_employee_loop == True:
        ...
        if user_selection in employee_id_list:
            ...
            employee_id_list.remove(employee_id_list[employee_id])
            f_name_list.remove(f_name_list[employee_id])
            l_name_list.remove(l_name_list[employee_id])
            email_list.remove(email_list[employee_id])
            salary_list.remove(salary_list[employee_id])
        else:
            print("ID not found, please try agan\n")
            time.sleep(1)
            delete_employee_loop = True
...
def give_bonus(employee_id_list, f_name_list, l_name_list, salary_list):
    ...
    while give_bonus_loop == True:
        ...
        try:
            ...
            "(In decimal notation. i.e. 25% = 0.25)\n>>> "))
            if raw_bonus < 1:
                bonus += 1
            data_file =open(FILE +".txt","w")
            for i in range(0,len(salary_list)):
                salary_list[i] = str(float(salary_list[i]) * bonus)
                data_file.write("ID: "+employee_id_list[i]+" Name: "+f_name_list[i]+" "+l_name_list[i]+" New salary: "+str(salary_list[i])+"\u20ac @ "+str((raw_bonus*100))+"% bonus\n")
            data_file.close()
        except:
